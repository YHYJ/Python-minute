...
这就好比中文和英文，说同一种语言的人可以交流，不同的语言之间就不行了
"""


"""
为了把全世界的所有不同类型的计算机都连接起来，就必须规定一套全球通用的协议
为了实现互联网的目标，互联网协议簇（Internet Protocol Suite）就是通用协议标准
...
但一台计算机可能同时接入到两个或更多的网络，比如路由器就会有两个或多个IP地址
...
因此路由器就负责决定如何把一个IP包转发出去
IP包的特点是按块发送，途径多个路由，但不保证能到达，也不保证按顺序到达
"""


"""
IPv4地址实际上是一个32位整数，以字符串表示如
192.168.0.1实际上是把32位整数按8位分组后的数字表示，目的是便于阅读

IPv6地址实际上是一个128位整数
它是目前使用的IPv4的升级版，以字符串表示类似于
...
TCP协议会通过握手建立连接，然后对每个IP包编号，确保对方按顺序收到
如果丢包就自动重发
"""
...
这样两个进程在两台计算机之间建立网络连接就需要各自的IP地址和各自的端口号
