...
"""lambda与def"""
# 1.lambda函数只是创建简单的函数对象，是一个函数的单行版本，但是这种语句由于性能的原因，调用的时候绕过函数的栈分配
# 2.lambda会创建一个函数对象，但不会把这个函数对象赋给一个标识符，而def则会把函数对象赋值给一个变量     #??????
# 3.lambda只是一个表达式，而def则是一个语句。lambda表达式运行起来像一个函数，当被调用时创建一个框架对象
